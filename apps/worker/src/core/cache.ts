...
/**
 * Cache key covers the question, the round count AND the full relevant
 * configuration (participant + moderator url/model/display_name), so an
 * admin edit or a different topology immediately produces different keys
...
export async function cacheKey(
  ...
  const digest = await crypto.subtle.digest(
    ...
    new TextEncoder().encode(
      `${question}|r${rounds}|${[...participantConfigs].sort().join(",")}|${moderatorConfig}`,
    ),
  );
